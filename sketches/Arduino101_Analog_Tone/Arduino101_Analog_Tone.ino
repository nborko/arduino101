// LED Level + Tone from Potentiometer Sketch for Arduino 101
// by Nick Borko
...
void setup() {
  int pin;
  // initialize pins 2-10 to be output pins
  for (pin = 2; pin <= 10; pin += 1) {
    pinMode(pin, OUTPUT);
  }
}
...
void loop() {
  ...
  // multiply the raw value to get an audible frequency
  int freq = raw * 4;
  // play the tone for 20ms
  tone(10, freq, 20);
